...
export function StatusSummary({ task }: { task: TaskDetail }) {
  ...
  return (
    <section className="grid rounded-lg border border-line bg-panel/85 px-4 md:grid-cols-2 lg:grid-cols-[2.4fr_1.35fr_.6fr_1fr_.7fr_.7fr_.65fr] lg:divide-x lg:divide-line">
      ...
      <Metric label="Status"><strong className={`inline-flex items-center gap-2 ${statusColors[task.status]}`}><ShieldCheck className="size-4" />{task.status.replaceAll("_", " ").toUpperCase()}</strong></Metric>
      ...
      <Metric label={auditComplete ? "Mode" : "Tests"}><strong className={auditComplete || (task.tests_total > 0 && task.tests_passed === task.tests_total) ? "text-mint" : "text-muted"}>{auditComplete ? "Read-only audit" : `${task.tests_passed} / ${task.tests_total}`}</strong></Metric>
